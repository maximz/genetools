...
def test_clr_normalization():
    """This tests genetools.stats.clr_normalize_rows and genetools.scanpy_helpers.clr_normalize_each_cell"""
    ...
    scanpy_helpers.clr_normalize_each_cell(sparse_adata)
    scanpy_helpers.clr_normalize_each_cell(dense_adata)
    ...
    assert np.allclose(manual_run_2nd_row, dense_adata.X[1, :], equal_nan=True)

    # Also test "inplace" parameter
    dense_adata2 = anndata.AnnData(X=dense)
    dense_adata3 = scanpy_helpers.clr_normalize_each_cell(dense_adata2, inplace=False)
